def report_generation():
        ...
        try:
            ...
            for student_id, student_data in report.items():
                ...
                for course in student_data["courses"]:
                    print(
                        f"| {course['course_id']:<12} | {course['assignment_grade']:<18} | {course['exam_grade']:<10} | {course['overall_grade']:<10} |")
                print("-" * 63)
                print()
            ...
            print("❌ Error: academic_performance.txt or students.txt file not found.")
    ...
    def course_attendance_report():
        try:
            ...
            for course_id, data in course_attendance.items():
                ...
                print(
                    f"| {course_id:<12} | {data['present']:<10} | {data['absent']:<10} | {attendance_percentage:<15.2f} |")
            print("=" * 60)
    ...
    def student_attendance_report():
        try:
            ...
            print("=" * 94)
            print(
                f"| {'Student ID':<12} | {'Name':<20} | {'Total Classes':<15} | {'Classes Attended':<15} | {'Attendance (%)':<15} |")
            print("-" * 94)

            for student_id, data in students_attendance.items():
                attendance_percentage = (data["total_classes_attended"] / data["total_classes"]) * 100 if data[
                                                                                                              "total_classes"] > 0 else 0
                student_name = students_name.get(student_id)
                print(
                    f"| {student_id:<12} | {student_name:<20} | {data["total_classes"]:<15} | {data["total_classes_attended"]:<16} | {attendance_percentage:<15} |")
            print("=" * 94)
        ...
        try:
            ...
            with open("../../Data/students_finance.txt", "r") as file:
                ...
                for line in file:
                    ...
                    student_name = students_name.get(student_id)
                    print(
                        f"| {student_id:<12} | {student_name:<20} | {total_fees:<12} | {fees_paid:<10} | {amount_left:<11} | {pay:<21} |")
